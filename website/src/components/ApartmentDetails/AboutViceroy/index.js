...
const AboutViceroy = ({data}) => {
  return (
    <section className="mx-5 mb-8 border-b border-b-border-light pb-9 md:mx-0 md:mb-9 md:pb-[42px] m-auto max-w-screen-2xl">
      <h3 className="mb-5 text-2xl font-bold">About {data.name}</h3>
      <p className="mb-4 text-gray-700">{data.description}</p>

      <h5 className="mb-2 text-lg font-semibold">Hours</h5>
      <p className="mb-4 text-gray-700">{data.hours}</p>
      <div className="mb-6">
        <h5 className="mb-2 text-lg font-semibold">Contact Information</h5>
        <ul>
          <li className="flex flex-wrap">
            <span className="font-medium mr-1">Phone: &nbsp;</span>
            <a href={`tel:+${data.phone}`} className="cursor-pointer text-brand hover:underline active:underline">
              {data?.phone}
            </a>
          </li>
          <li className="flex flex-wrap">
