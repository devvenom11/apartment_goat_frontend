...
const HeroSlider = ({  images}) => {
  const isMultipleImages = images && images.length > 1;
  ...
  function PrevArrow(props) {
    const { className, style, onClick } = props;
    return (
      <div className={className} style={{ ...style, display: "block", zIndex: "4" }} onClick={onClick}>
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const setting = {
    ...
    slidesToShow: 2.5,
    slidesToScroll: 1,
    nextArrow: isMultipleImages ? <NextArrow /> : null,  
    prevArrow: isMultipleImages ? <PrevArrow /> : null, 
    responsive: [
      {
        breakpoint: 1024,
        settings: {
      ...
      <Slider {...setting}>
        {images?.map((item, idx) => (
          <div key={idx} className="relative ">
            ...
            <img src={`${IMAGE_PATH}${item.file}.1018x328.webp`} alt={item?.alt} className="w-full min-h-[211px] sm:min-h-[328px] sm:max-h-[328px] object-cover" />
          </div>
        ))}
      </Slider>
    </div>
